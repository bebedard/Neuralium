...
namespace Neuralium.Shell.Classes.General {
	...
	public interface IRpcClientEvents {
		...
		Task TransactionReceived(string transactionId, object transaction);
	...
	public class RpcProvider<RPC_HUB, RCP_CLIENT> : IRpcProvider<RPC_HUB, RCP_CLIENT>
		...
		public Task<object> QuerySystemInfo() {
			...
			systemMode = 2;
				...
				return Task.FromResult((object) new {version = GlobalSettings.SoftwareVersion.ToString(), mode = systemMode});
			} catch(Exception ex) {
		...
		public Task<List<object>> QuerySupportedChains() {
			try {
				...
				return Task.FromResult(globalsService.SupportedChains.Select(c => new {Id = c.Key.Value, c.Value.Name, c.Value.Enabled, c.Value.Started}).Cast<object>().ToList());
			} catch(Exception ex) {
	...
	#region Utils
		...
		private void TimerCallback(object state) {
			...
			lock(this.locker) {
				timedOuts = this.longRunningEvents.Where(e => e.Value.Timeout >= DateTime.Now).Select(e => e.Key).ToArray();
			}
		...
		public class LongRunningEvents {
			...
			public void SlideTimeout() {
				this.Timeout = DateTime.Now + this.spanTimeout;
			}
		}
