...
namespace Neuralia.Data.HashFunction.xxHash.Utilities.UnifiedData
{
            ...
            IDisposable
    {
        ...
        public override long Length { get { return this._data.Length; } }
        ...
        public StreamData(RecyclableMemoryStream data)
        {
        ...
        public override void ForEachRead(Action<byte[], int, int> action, CancellationToken cancellationToken)
        {
            ...
            while ((bytesRead = this._data.Read(buffer, 0, buffer.Length)) > 0)
            {
                cancellationToken.ThrowIfCancellationRequested();
        ...
        public override async Task ForEachReadAsync(Action<byte[], int, int> action, CancellationToken cancellationToken)
        {
            ...
            while ((bytesRead = await this._data.ReadAsync(buffer, 0, buffer.Length, cancellationToken).ConfigureAwait(false)) > 0)
            {
                cancellationToken.ThrowIfCancellationRequested();
        ...
        public override void ForEachGroup(int groupSize, Action<byte[], int, int> action, Action<byte[], int, int> remainderAction, CancellationToken cancellationToken)
        {
            ...
            while ((currentLength = this._data.Read(buffer, position, buffer.Length - position)) > 0)
            {
                cancellationToken.ThrowIfCancellationRequested();
        ...
        public override async Task ForEachGroupAsync(int groupSize, Action<byte[], int, int> action, Action<byte[], int, int> remainderAction, CancellationToken cancellationToken)
        {
            ...
            while ((currentLength = await this._data.ReadAsync(buffer, position, buffer.Length - position, cancellationToken).ConfigureAwait(false)) > 0)
            {
                cancellationToken.ThrowIfCancellationRequested();
        ...
        public override byte[] ToArray(CancellationToken cancellationToken)
        {
            ...
            using (var ms = new RecyclableMemoryStreamManager().GetStream())
            {
                var buffer = new byte[this.BufferSize];
                int bytesRead;
        ...
        public override byte[] Data {
            get { return this._data.GetBuffer(); }
        }
        ...
        public override async Task<byte[]> ToArrayAsync(CancellationToken cancellationToken)
        {
            ...
            {
                await this._data.CopyToAsync(ms, this.BufferSize, cancellationToken)
