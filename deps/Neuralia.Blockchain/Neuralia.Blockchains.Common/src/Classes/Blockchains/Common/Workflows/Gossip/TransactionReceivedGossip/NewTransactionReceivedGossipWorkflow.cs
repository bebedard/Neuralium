...
namespace Neuralia.Blockchains.Common.Classes.Blockchains.Common.Workflows.Gossip.TransactionReceivedGossip {
	...
	public abstract class NewTransactionReceivedGossipWorkflow<CENTRAL_COORDINATOR, CHAIN_COMPONENT_PROVIDER, GOSSIP_MESSAGE_SET, GOSSIP_WORKFLOW_TRIGGER_MESSAGE, EVENT_ENVELOPE_TYPE> : GossipChainWorkflow<CENTRAL_COORDINATOR, CHAIN_COMPONENT_PROVIDER, GOSSIP_MESSAGE_SET, GOSSIP_WORKFLOW_TRIGGER_MESSAGE, EVENT_ENVELOPE_TYPE>
		...
		public NewTransactionReceivedGossipWorkflow(BlockchainType chainType, CENTRAL_COORDINATOR centralCoordinator, GOSSIP_MESSAGE_SET triggerMessage, PeerConnection peerConnectionn) : base(chainType, centralCoordinator, triggerMessage, peerConnectionn) {
		...
		protected override void PerformWork() {
			...
			blockchainTask.SetAction((blockchainService, taskRoutingContext) => {
				...
				blockchainService.InsertGossipTransaction((ITransactionEnvelope) this.triggerMessage.Message.Envelope);
